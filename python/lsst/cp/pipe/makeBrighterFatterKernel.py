...
from lsst.ip.isr import (IsrTask, BrighterFatterKernel)
...
class BrighterFatterKernelSolveTask(pipeBase.PipelineTask, pipeBase.CmdLineTask):
    ...
    def runQuantum(self, buterQC, inputRefs, outputRefs):
    ...
    def run(self, inputPtc, camera, inputDims):
        ...
        if self.config.level == 'AMP':
            ...
            bfk = BrighterFatterKernel(camera=camera, level=self.config.level)

        bfk.means = inputPtc.finalMeans
        bfk.rawMeans = inputPtc.rawMeans
        bfk.rawXcorrs = inputPtc.covariances
        bfk.gain = inputPtc.gain
        bfk.gainErr = inputPtc.gainErr
        bfk.noise = inputPtc.noise
        bfk.noiseErr = inputPtc.noiseErr
        ...
        for amp in detector:
            ...
            fluxes = bfk.means[ampName]
            xCorrList = [np.array(xcor) for xcor in bfk.rawXcorrs[ampName]]

            meanXcorr = np.zeros_like(xCorrList[0])
            for xcorr, flux in zip(xCorrList, fluxes):
                q = xcorr
                q[0][0] -= 2.0*flux
                q /= -2.0*(flux**2)
                meanXcorr += xcorr / flux
            meanXcorr /= len(fluxes)

            # Full sum:
            # To match previous definitions, pad by one element.
            meanXcorr = np.pad(meanXcorr, ((0, 1)))
            preKernel = self._tileArray(meanXcorr)
            totalSum = np.sum(preKernel)
            meanXcorr[0][0] -= totalSum

            bfk.meanXCorrs[ampName] = meanXcorr
            preKernel = self._tileArray(meanXcorr)
            postKernel = self.successiveOverRelax(preKernel)
            bfk.ampKernels[ampName] = postKernel
            print(ampName, gain, meanXcorr.sum(), postKernel.shape)
...
#         gains : `dict`
#             A dictionary of the amplifier gain values, keyed by amplifier name.
#         level : `str`
...
#         -------
#         gains : `dict`
#             A dict of the as-calculated amplifier gain values, keyed
#             by amp name
#         ptcData : `dict`
#             PTC results used to estimate gains.
...
#         return gains, ptcResults, nomGains
